...
**Answer** Two main things: first, conference realignment. We can see App State there in the head, but schools like Texas, Oklahoma, Oregon, and USC were impacted heavier from 2022-2024 with joining new conferences in the NIL era. The more important issue for us though is 2020, when most stadiums didn't allow spectators because of the coronavirus. That'll make year-over-year and change over time data harder to calculate. 
...
**Answer** Once again, conference realignment is a big part of it. However, a bigger factor is that Ohio State, Indiana, and Texas all hosted an extra game in 2024 because of the modified CFP format. While SMU didn't host a game, they still made the CFP, bringing their fans out far more than in years past. 
...
**Answer** The main thing that stands out to me is the discrepency between the number of shots on goal and goals amongst teams that are right next to each other based on percentage. Sure, Seton Hall is scoring at a slightly higher rate than Colorodo, but Colorado has 30 more shots on goal and 14 more goals. I'm going to narrow it down slightly further by just doing the teams who are over the median of total shots on goal. 

```{r}
median_total_sog <- matches_2025 |>
  filter(date <= "2025-09-09") |>
  group_by(team) |> 
  summarise(
    total_on_goal = sum(so_g),
...
median_total_sog |>
  mutate(median_total_on_goal = median(total_on_goal))

median_total_sog |> 
  filter(total_on_goal >= "36.5") |>
  arrange(desc(percent))
```
## I think this is a more accurate depiction of who was performing the best on offense at this point in the season. 
